...
function expandShortcodes(html, lang, pagePath) {
  ...
  const otherLabel = lang === 'en' ? 'Français' : 'English';
  const otherFlag = lang === 'en' ? '🇫🇷' : '🇬🇧';
  const selfFlag = lang === 'en' ? '🇬🇧' : '🇫🇷';
  const selfLabel = lang === 'en' ? 'English' : 'Français';
...
<form class="mc-form" onsubmit="event.preventDefault(); alert(${lang === 'en' ? "'Preview only — form submissions are disabled locally.'" : "'Aperçu uniquement — l’envoi de formulaires est désactivé en local.'"});">
  ...
  <p class="mc-form__note">${lang === 'en' ? 'Preview form — contact@maisoncalista.com' : 'Formulaire d’aperçu — contact@maisoncalista.com'}</p>
</form>`);
...
<head>
  ...
  <style>
    /* Preview shell: WordPress-like CSS variables from theme.json */
    :root {
    ...
    @media (max-width: 960px) {
      .mc-header__nav.is-open .wp-block-navigation__container,
...
const server = http.createServer((req, res) => {
  try {
    ...
    // Convenience aliases for assets referenced without /theme prefix in some CSS
    ...
    const html = buildPage(hit.page, hit.lang);
