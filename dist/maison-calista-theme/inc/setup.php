 ...
 * @return string
 */
function maison_calista_contact_notices( $content ) {
	...
	if ( 'sent' === $status ) {
		$msg = ( function_exists( 'maison_calista_is_english' ) && maison_calista_is_english() )
			...
			: "Merci. Votre message a bien été envoyé. Nous vous répondrons avec attention et discrétion.";
		...
		$msg = ( function_exists( 'maison_calista_is_english' ) && maison_calista_is_english() )
			...
			: "Veuillez remplir tous les champs obligatoires et accepter la notice de confidentialité.";
 ...
 *
 * @param mixed $content Block / post content.
 * @return string
 */
function maison_calista_replace_asset_placeholders( $content ) {
	if ( ! is_string( $content ) || '' === $content ) {
		return is_string( $content ) ? $content : '';
	}

	$home = ( function_exists( 'maison_calista_lang_home_url' ) )
		? maison_calista_lang_home_url()
 ...
 * @param mixed $block         Parsed block array.
 * @return string
 */
function maison_calista_do_shortcodes_in_html_blocks( $block_content, $block = array() ) {
	if ( ! is_string( $block_content ) ) {
		return '';
	}
	if ( is_array( $block ) && isset( $block['blockName'] ) && 'core/html' === $block['blockName'] ) {
...
/**
 * Neutralize WordPress layout constraints that break full-bleed preview sections.
 */
function maison_calista_body_layout_class( $classes ) {
	if ( ! is_array( $classes ) ) {
		$classes = array();
	}
	$classes[] = 'mc-preview-parity';
	return $classes;
}
add_filter( 'body_class', 'maison_calista_body_layout_class', 20 );
